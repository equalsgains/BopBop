...
function getcourseObj() {
    ...
    $.ajax({
        url: 'https://lquintanilla.instructure.com/api/v1/courses/18/',
        type: 'GET',
        data: 'per_page=100',
...
function getSectionsObj() {
    $.ajax({
        url: 'https://lquintanilla.instructure.com/api/v1/courses/18/sections',
        type: 'GET',
        data: 'per_page=100',
        beforeSend: function (xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + access_token);
...
function spitSectionsInfo() {
    for (var i = 0; i < s.length; i++) {
        ...
        $('#container').append('<a class="slide-fade show" href=https://' + document.domain + '/courses/' + course + '/sections/' + s[i].id + ' target="_blank"><div class="panel panel-blue"><div class="panel-heading"><div class="row"><div class="col-xs-3"><h1 class="titles">Section</h1><i class="fa fa-tasks fa-2x"></i></div><div class="col-xs-9 text-right"><div class="huge">' + s[i].name + '</div><li><span class="StartEnd">Start</span> ' + sstart + '</li><li><span class="StartEnd">End</span> ' + send + '</li></a>');
    };
};
